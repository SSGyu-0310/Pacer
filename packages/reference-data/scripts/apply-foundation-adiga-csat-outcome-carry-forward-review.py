...
OUTPUT_SUMMARY = "foundation_adiga_csat_outcome_carry_forward_review_summary.json"

REVIEWER = "codex-adiga-csat-outcome-carry-forward-audit-v1"
APPROVAL_NOTE = (
    "Strict ADIGA CSAT outcome carry-forward audit: foundation row matched ADIGA "
    "candidateSha256 and numeric values, extracted table grid row contained the admission "
    "unit and all core values, and recruitment group was verified by scanning upward to "
    "the nearest explicit group cell in the same table."
)
...
def main() -> None:
    ...
    for row in decision_rows:
        ...
        if normalize_text(row.get("reviewOutcome")) != "pending":
            continue
        result = verify_adiga_carry_forward_row(
            adiga_dir,
            row,
            historical_outcomes,
            adiga_candidates,
    ...
    print(
        "foundation ADIGA CSAT outcome carry-forward review complete. "
...
def verify_adiga_carry_forward_row(
    adiga_dir: Path,
    ...
    candidate = adiga_candidates.get(normalize_text(outcome.get("sourceCandidateSha256")))
    ...
    expected_group = GROUP_TEXT.get(normalize_text(outcome.get("recruitmentGroup")), "")
    if expected_group and not group_verified_by_carry_forward(grid, row_index - 1, expected_group):
        return result(decision_row, "group_not_carried_forward", "Recruitment group could not be verified by upward scan.")
    ...
    return result(decision_row, "matched", "Strict ADIGA carry-forward group and value checks passed.")


def group_verified_by_carry_forward(grid: list[Any], row_index_zero_based: int, expected_group: str) -> bool:
    for index in range(row_index_zero_based, -1, -1):
        row = grid[index]
        if not isinstance(row, list):
            continue
        row_text = " ".join(clean_cell(cell) for cell in row)
        groups_in_row = [group for group in ("가군", "나군", "다군") if group in row_text]
        if not groups_in_row:
            continue
        return groups_in_row[0] == expected_group
    return False
...
def summarize(
    *,
    ...
    return {
        ...
        "artifactType": "foundation_adiga_csat_outcome_carry_forward_review_summary",
        ...
        "notes": [
            ...
            "Rows are approved only when foundation values match the ADIGA candidate row and the extracted table row contains all core numeric values.",
            "Recruitment group may be carried forward only from the nearest explicit 가군/나군/다군 row above the target row in the same extracted table.",
        ],
    }
