...
DEFAULT_OFFICE_DOCUMENT_SOURCE_MANIFEST = (
    "packages/reference-data/data/public/university-admission-sites/extracted/"
    "university_admission_office_document_sources_manifest_2027.jsonl"
)
...
def main() -> None:
    ...
    source_manifest_path = resolve(repo_root, args.office_document_source_manifest)
    ...
    for source in source_rows:
        if source.get("extractionStatus") not in {"extracted", "low_text", "reused_duplicate_sha256"}:
            continue
    ...
    write_jsonl(
        output_dir / f"university_admission_office_document_snippets_{args.year}.jsonl",
        snippets,
    )
    write_csv_index(
        output_dir / f"university_admission_office_document_snippet_index_{args.year}.csv",
        snippets,
    )
    summary = summarize(args.year, source_rows, snippets)
    (
        output_dir / f"university_admission_office_document_snippets_summary_{args.year}.json"
    ).write_text(
    ...
    print(
        "university admission office document snippet extraction complete. "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--office-document-source-manifest",
        default=DEFAULT_OFFICE_DOCUMENT_SOURCE_MANIFEST,
    )
...
def build_snippet(
    *,
    ...
    snippet_sha = hashlib.sha256(
        "|".join(
            [
                str(source.get("rawOfficeDocumentSha256") or ""),
                rule["role"],
                str(start_line),
                str(end_line),
                snippet_text,
    ...
    return {
        ...
        "artifactType": "admission_office_document_text_snippet",
        ...
        "rawOfficeDocumentPath": source.get("rawOfficeDocumentPath"),
        "rawOfficeDocumentSha256": source.get("rawOfficeDocumentSha256"),
        "fileExtension": source.get("fileExtension"),
...
def summarize(
    ...
    eligible_sources = [
        ...
        if row.get("extractionStatus") in {"extracted", "low_text", "reused_duplicate_sha256"}
    ]
    source_rows_with_snippets = {str(row.get("sourceRowKey") or "") for row in snippets}
    unique_documents_with_snippets = {
        str(row.get("rawOfficeDocumentSha256") or "") for row in snippets
    }
    return {
        "provider": "university-admission-office",
        ...
        "uniqueOfficeDocumentsWithSnippets": len(unique_documents_with_snippets),
        "snippets": len(snippets),
        "uniqueSnippetSha256": len({str(row.get("snippetSha256") or "") for row in snippets}),
        "bySnippetRole": count_by(snippets, "snippetRole"),
        "byFileExtension": count_by(snippets, "fileExtension"),
        "bySourceLinkRole": count_by(snippets, "sourceLinkRole"),
        "byDetectedDocumentRole": count_by(snippets, "detectedDocumentRole"),
        "notes": [
            "Snippets are keyword-scored candidates extracted from DOC/DOCX/PPT/PPTX text output.",
            "Snippet text is capped for manifest size; textPath/rawOfficeDocumentPath retain the source for full review.",
